...
module.exports = async options => {
	const {stdout} = await execFile(activeWinBin, getArguments(options));
	return parseMac(stdout);
};

module.exports.sync = options => {
	const stdout = childProcess.execFileSync(activeWinBin, getArguments(options), {encoding: 'utf8'});
	return parseMac(stdout);
};

module.exports.getOpenWindows = async options => {
	const {stdout} = await execFile(activeWinBin, [...getArguments(options), '--open-windows-list']);
	return parseMac(stdout);
};

module.exports.getOpenWindowsSync = options => {
	const stdout = childProcess.execFileSync(activeWinBin, [...getArguments(options), '--open-windows-list'], {encoding: 'utf8'});
	return parseMac(stdout);
};
...
module.exports.isAccessGranted = () => {
	let result;
	try {
		result = childProcess.execFileSync(isAccessGrantedBin);
	} catch(err) {
		result = "{}";
	}
	try {
